...
GameState run_menu() {
    ...
    BeginDrawing();
       ...
       DrawTexturePro(menu_background, src, dest, (Vector2){ 0, 0 }, 0.0f, Fade(WHITE, 0.8f));
        ...
        float brilho = (sin(tempo * 1.5f) + 1) / 2.0f; 
...
GameState creditos() {
    ...
    DrawRectangle(0, 0, TELA_COMP, TELA_ALTURA, Fade(BLUE, 0.15f)); //filtro azul
...
GameState gameover() {
    ...
    BeginDrawing();
        ...
        // Mensagem principal
        DrawText("GAME OVER!", TELA_COMP / 2 - MeasureText("GAME OVER!", 80) / 2, TELA_ALTURA / 2 - 50, 80, RED);
        
        // Mensagem de instrução
        DrawText("Pressione ENTER para voltar ao Menu", TELA_COMP / 2 - MeasureText("Pressione ENTER para voltar ao Menu", 20) / 2, TELA_ALTURA / 2 + 50, 20, RAYWHITE);
